...
SELECT COUNT(CO.PIZZA_ID) AS SUCCESSFUL_ORDERS
,PN.PIZZA_NAME
FROM RUNNER_ORDERS AS RO
INNER JOIN CUSTOMER_ORDERS AS CO
ON CO.ORDER_ID = RO.ORDER_ID
...
SELECT COUNT(CO.PIZZA_ID) AS CUSTOMER_ORDERS
,PN.PIZZA_NAME
,CO.CUSTOMER_ID
...
SELECT COUNT(CO.PIZZA_ID) AS NUMBER_OF_PIZZAS
,CO.ORDER_ID
FROM RUNNER_ORDERS AS RO
INNER JOIN CUSTOMER_ORDERS AS CO
ON CO.ORDER_ID = RO.ORDER_ID
...
WHEN 
(
  (EXCLUSIONS<>'null' AND LENGTH(EXCLUSIONS)>0) OR (EXTRAS<>'null' AND EXTRAS IS NOT NULL AND LENGTH(EXTRAS)>0)
)=TRUE
THEN 1 ELSE 0
END)
AS CHANGES
